...
class DeformConv2d(nn.Module):
    def __init__(self, inc, outc, kernel_size=3, padding=1, stride=1, bias=None, modulation=True):
        ...
        self.p_conv.register_full_backward_hook(self._set_lr)  # 修改为 full backward hook
        ...
        if modulation:
            ...
            self.m_conv.register_full_backward_hook(self._set_lr)  # 修改为 full backward hook
    ...
    def forward(self, x):
        ...
        # modulation
        if self.modulation:
            m = m.permute(0, 2, 3, 1).contiguous()
            m = m.unsqueeze(dim=1)
    ...
    def _get_p_0(self, h, w, N, dtype):
        ...
        p_0_x = torch.flatten(p_0_x).reshape(1, 1, h, w).repeat(1, N, 1, 1)
    ...
    def _reshape_x_offset(x_offset, ks):
        ...
        x_offset = torch.cat([x_offset[..., s:s+ks].contiguous().reshape(b, c, h, w*ks) for s in range(0, N, ks)], dim=-1)
