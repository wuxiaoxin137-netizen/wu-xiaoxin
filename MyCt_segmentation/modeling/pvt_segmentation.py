...
class PyramidVisionTransformerV2(nn.Module):
    def __init__(self, img_size=512, patch_size=16, in_chans=3, num_classes=1, embed_dims=[64, 128, 320, 512],
...
class PVT_UNet(nn.Module):
    def __init__(self):
        ...
        # Assuming Pyramid Vision Transformer (PVT) as the encoder
        self.encoder = get_pvt_v2_b2_li_encoder()

        # Decoder part of U-Net
        self.up1 = nn.ConvTranspose2d(512, 320, kernel_size=2, stride=2)
        self.conv1 = DoubleConv(640, 320)
        ...
        # New upsampling layer to get 512x512 resolution
    ...
    def forward(self, x):
        ...
        s1, s2, s3, s4 = encoder_outputs

        x = self.up1(s4)
        x = TF.resize(x, size=s3.shape[2:])
        x = torch.cat([s3, x], dim=1)
        x = self.conv1(x)

        x = self.up2(x)
        x = TF.resize(x, size=s2.shape[2:])
        x = torch.cat([s2, x], dim=1)
        x = self.conv2(x)

        x = self.up3(x)
        x = TF.resize(x, size=s1.shape[2:])
        x = torch.cat([s1, x], dim=1)
        x = self.conv3(x)

        x = self.up4(x)

        # Final upsampling to reach 512x512
        x = self.up5(x)

        return self.final_conv(x)

def get_pvt_v2_b2_li_encoder():
...
# # 实例化编码器
# encoder = get_pvt_v2_b2_li_encoder()
#
# # 创建模型
# model = PVT_UNet()
# # model = PVT_UNet(encoder=encoder, out_channels=1)
#
# # 输入示例
