...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x, skip, H, W):
        x_up = self.Upsample(x, H, W)
        if skip is not None:  # 如果存在跳跃连接
            # 打印 x_up 和 skip 的形状
            # print(f"x_up Shape: {x_up.shape}, skip Shape: {skip.shape}")
            x = x_up + skip
        else:  # 没有跳跃连接时，仅使用上采样结果
            x = x_up
        H, W = H * 2, W * 2
...
class encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            # 添加打印输出信息
            # print(f"Encoder Layer {i}: Output Shape {x_out.shape}")
...
class decoder(nn.Module):
    ...
    def forward(self, x, skips):
        ...
        for i in range(self.num_layers)[::-1]:
            ...
            # skip = skips[skip_idx] if skip_idx >= 0 else None  # 确保索引在范围内
            # print(f"x_up Shape: {x.shape}, skip Shape: {skip.shape if skip is not None else None}")
            #
            #
            # # 跳跃连接部分注释
            # x, H, W, = layer(x, skips[i], H, W)

            # 检查 skips 是否足够，并安全访问索引
            if i < len(skips):
                skip = skips[-(i + 1)]  # 调整为 -(i + 1) 确保 skip 正确匹配
            else:
                skip = None

            # 打印调试信息
            # if skip is not None:
            #     print(f"x_up Shape: {x.shape}, skip Shape: {skip.shape}")
            # else:
            #     print(f"x_up Shape: {x.shape}, skip: None")

            x, H, W = layer(x, skip, H, W)
            # 添加打印输出信息
            # print(f"Decoder Layer {i}: x Shape {x.shape}")
            outs.append(x)
        return outs
...
#         for i in range(self.num_layers)[::-1]:
#             layer = self.layers[i]
#             skip = skips[i] if skips is not None else None  # 检查 skips 是否为 None
#             x, H, W = layer(x, skip, H, W)
#             # 打印输出信息
#             print(f"Decoder Layer {i}: x Shape {x.shape}")
#             outs.append(x)
...
class UNET(nn.Module):
    def __init__(self,
                 ...
                 num_classes=1,
                 embed_dim=96,
                 depths_encoder=(3, 3, 3, 3),
                 depths_decoder=(3, 3, 3),
                 patch_size=(4, 4),
                 img_size=(512, 512),
        ...
        self.encoder = encoder(
            ...
            channelAttention_reduce=channel_attention_reduce
        )
        ...
        # 定义最终 Patch Expanding 层
        self.final_patch_expanding = final_patch_expanding(
            dim=embed_dim,
    ...
    def forward(self, x):
        ...
        # Decoder
        decoded = self.decoder(neck, skips[:-1][::-1])
        # decoded = self.decoder(neck, skips=None)  # 使用实例化后的 self.decoder

        # Final Output
        out = self.final_patch_expanding(decoded[-1])
        return out
...
#     # 定义输入图像
#     x = torch.randn(1, 3, 512, 512)  # Batch size 1, 3 通道，512x512 尺寸
#
#     # 实例化模型
#     model = UNET(
#         input_channels=3,
...
#     # 前向推理
#     output = model(x)
#     print(f"Output shape: {output.shape}")  # 预期输出：torch.Size([1, 1, 512, 512])
