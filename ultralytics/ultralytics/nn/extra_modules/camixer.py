...
def flow_warp(x, flow, interp_mode='bilinear', padding_mode='zeros', align_corners=True):
    ...
    _, _, h, w = x.size()
    # create mesh grid
    # 使用 .type_as(x) 确保了 grid_y 和 grid_x 的 device 和 dtype 初始就与 x 一致
    grid_y, grid_x = torch.meshgrid(torch.arange(0, h).type_as(x), torch.arange(0, w).type_as(x))
    # grid 拼接后，其类型也应该与 x 一致。 .float() 会强制转为 float32，可能会导致问题。
    # 我们先移除 .float()，让类型跟随 grid_y 和 grid_x。
    grid = torch.stack((grid_x, grid_y), 2)
    grid.requires_grad = False

    # vgrid 的计算中，grid 和 flow 的类型需要一致。flow 的类型由外部传入，通常与 x 一致。
    vgrid = grid + flow
    ...
    # =========================================================================
    # --- 【核心修改点】 ---
    # 在调用 F.grid_sample 之前，确保 vgrid_scaled 的数据类型与输入 x 完全一致。
    # 这样可以解决在混合精度（AMP）训练中可能出现的 Half vs Float 的类型冲突问题。
    # =========================================================================
    output = F.grid_sample(x, vgrid_scaled.to(x.dtype), mode=interp_mode, padding_mode=padding_mode,
