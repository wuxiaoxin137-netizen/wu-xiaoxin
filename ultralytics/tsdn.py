...
class DilatedOCA(nn.Module):
    ...
    def forward(self, x):
        ...
        # =========================================================================
        # --- 【核心修改点】 ---
        # 对输入张量 qs 进行动态填充，以确保其高和宽可以被 window_size 整除。
        # =========================================================================
        H_ori, W_ori = h, w
        ws = self.window_size
        pad_h = (ws - H_ori % ws) % ws
        pad_w = (ws - W_ori % ws) % ws
        if pad_h > 0 or pad_w > 0:
            # (pad_left, pad_right, pad_top, pad_bottom)
            qs = F.pad(qs, (0, pad_w, 0, pad_h))

        # 获取填充后的新尺寸
        b, c, h_pad, w_pad = qs.shape

        # spatial attention
        # 使用填充后的尺寸进行 rearrange
        ...
        # 使用填充后的尺寸 h_pad 和 w_pad 进行 rearrange
        out = rearrange(out, '(b h w head) (p1 p2) c -> b (head c) (h p1) (w p2)', head=self.num_spatial_heads,
                        h=h_pad // self.window_size, w=w_pad // self.window_size, p1=self.window_size,
                        p2=self.window_size)

        # =========================================================================
        # --- 【核心修改点 2】 ---
        # 如果进行了填充，需要在最后将输出裁剪回原始尺寸。
        # =========================================================================
        if pad_h > 0 or pad_w > 0:
            out = out[:, :, :H_ori, :W_ori]
...
class DTAB(nn.Module):
    def __init__(self, dim, window_size=4, overlap_ratio=0.5, num_channel_heads=4, num_spatial_heads=2,
                 spatial_dim_head=16, ffn_expansion_factor=1, bias=False, LayerNorm_type='BiasFree'):
